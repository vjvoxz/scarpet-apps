...
_spread_down(pos, lichen_block) -> (
    ...
    if(_is_air(below_block),
        if(property(state, 'north') == 'true' && rand(1) < 0.5,
            set(below, 'glow_lichen', 'north', 'true')
        );
        if(property(state, 'south') == 'true' && rand(1) < 0.5,
            set(below, 'glow_lichen', 'south', 'true')
        );
        if(property(state, 'east') == 'true' && rand(1) < 0.5,
            set(below, 'glow_lichen', 'east', 'true')
        );
        if(property(state, 'west') == 'true' && rand(1) < 0.5,
            set(below, 'glow_lichen', 'west', 'true')
        );
    ,
        if(property(state, 'north') == 'true' && rand(1) < 0.5 && _can_attach([x, y-1, z-1]),
            set(below, 'glow_lichen', 'north', 'true')
        );
        if(property(state, 'south') == 'true' && rand(1) < 0.5 && _can_attach([x, y-1, z+1]),
            set(below, 'glow_lichen', 'south', 'true')
        );
        if(property(state, 'east') == 'true' && rand(1) < 0.5 && _can_attach([x+1, y-1, z]),
            set(below, 'glow_lichen', 'east', 'true')
        );
        if(property(state, 'west') == 'true' && rand(1) < 0.5 && _can_attach([x-1, y-1, z]),
            set(below, 'glow_lichen', 'west', 'true')
        );
    );
);
...
_spread_horizontal(pos, lichen_block) -> (
    ...
    if(property(state, 'north') != 'true' && _is_air([x, y, z-1]),
        _spread_horizontal_direction(pos, state, 'north', [x, y, z-1])
    );
    if(property(state, 'south') != 'true' && _is_air([x, y, z+1]),
        _spread_horizontal_direction(pos, state, 'south', [x, y, z+1])
    );
    if(property(state, 'east') != 'true' && _is_air([x+1, y, z]),
        _spread_horizontal_direction(pos, state, 'east', [x+1, y, z])
    );
    if(property(state, 'west') != 'true' && _is_air([x-1, y, z]),
        _spread_horizontal_direction(pos, state, 'west', [x-1, y, z])
    );
);

_spread_horizontal_direction(pos, state, direction, target) -> (
    [tx, ty, tz] = target;
    
    if(direction == 'north',
        if(property(state, 'east') == 'true' && _can_attach([tx+1, ty, tz]),
            set(target, 'glow_lichen', 'east', 'true')
        );
        if(property(state, 'west') == 'true' && _can_attach([tx-1, ty, tz]),
    ...
    if(direction == 'east',
        if(property(state, 'north') == 'true' && _can_attach([tx, ty, tz-1]),
            set(target, 'glow_lichen', 'north', 'true')
        );
        if(property(state, 'south') == 'true' && _can_attach([tx, ty, tz+1]),
